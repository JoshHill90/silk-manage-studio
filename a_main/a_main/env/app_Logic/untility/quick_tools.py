...
ven = current_dir / "../.env"
load_dotenv(ven)
keyStr = str(os.environ["PM_STRIPE_KEY"])
...
class DateFunction:
	...
	def date_time_now(self):
		date_and_time = datetime.now()
		dates_obj = date_and_time.date()
		times_obj = date_and_time.time()
		date_str = str(dates_obj)
		time_str = str(times_obj)
...
class ViewQueryHelper:
	...
	def image_query(self, request, image_object, gal):
		...
		# Apply filters
		if current_gal == 'True':
			image_object = image_object.display_set.filter(display=gal)

		if project_query :
			image_object = image_object.filter(project_id__name__icontains=project_query)

		if client_query:
			image_object = image_object.filter(
